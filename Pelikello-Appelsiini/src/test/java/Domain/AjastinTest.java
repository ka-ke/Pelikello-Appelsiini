...
public class AjastinTest {
    ...
    @Test
    public void tuntiVaheneeOikein() {
        Ajastin tunti = new Ajastin(1, 0, 0);
        tunti.aikaKuluu();
        assertEquals("00:59:59", tunti.toString());
    }

    @Test
    public void aikaKasvaaKunSitaMenee() {
        ajastin.aikaaMenee();
        assertEquals("00:01:11", ajastin.toString());
    }

    @Test
    public void minuuttiKasvaaOikein() {
        for (int i = 0; i < 60; i++) {
            ajastin.aikaaMenee();
        }
        assertEquals("00:02:10", ajastin.toString());
    }

    @Test
    public void tuntiKasvaaOikein() {
        Ajastin tunti = new Ajastin(0, 59, 59);
        tunti.aikaaMenee();
        assertEquals("01:00:00", tunti.toString());
    }
